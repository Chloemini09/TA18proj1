      ...
      async function fetchUVData() {
        ...
        try {
          ...
          if (!geoResponse.data || geoResponse.data.length === 0) {
            console.error('Location not found in geo API response');
            // 如果无法获取位置，使用默认值
            uvIndex.value = 6;
          ...
          // 使用当前天气 API 获取 UV 指数 (OpenWeather 更改了 API)
          ...
          console.log('Calling OneCall API:', oneCallUrl);
          
          try {
            const oneCallResponse = await axios.get(oneCallUrl);
            console.log('OneCall API response:', oneCallResponse.data);
            
            // 从 OneCall API 获取 UV 指数
            if (oneCallResponse.data && oneCallResponse.data.current && oneCallResponse.data.current.uvi !== undefined) {
              ...
              console.log('UV Index retrieved from OneCall API:', uvIndex.value);
            } else {
              // 如果 OneCall API 无法获取 UV 指数，使用备用方法
              uvIndex.value = getEstimatedUVIndex(weatherResponse.data);
              console.log('Estimated UV Index:', uvIndex.value);
            }
          } catch (oneCallError) {
            console.error('Error fetching from OneCall API:', oneCallError);
            // 如果 OneCall API 调用失败，使用天气数据估算 UV 指数
            uvIndex.value = getEstimatedUVIndex(weatherResponse.data);
            console.log('Estimated UV Index due to OneCall API error:', uvIndex.value);
          }
          ...
          console.log('UV data saved to localStorage:', uvData);
          ...
          // 使用默认 UV 指数
          uvIndex.value = 6;
          console.log('Using fallback UV Index value:', uvIndex.value);
          updateRecommendedItems();
        } finally {
