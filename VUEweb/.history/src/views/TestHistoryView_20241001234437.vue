<template>
  <div class="test-history-container">
    ...
    <div v-if="filteredTestRecords.length === 0">
    ...
    <table v-else>
      ...
      <tbody>
        <tr v-for="test in paginatedRecords" :key="test.date">
          <td>{{ test.date }}</td>
          <td>{{ test.score }}</td>
        </tr>
      </tbody>
    ...
    try {
      ...
      if (!querySnapshot.empty) {
        testRecords.value = querySnapshot.docs.map(doc => {
          ...
          return {
            ...data,
            date: data.date.toDate() // 确保日期是 JavaScript Date 对象
          };
        });
      } else {
...
const filteredTestRecords = computed(() => {
  return testRecords.value.filter(record => {
    const recordDate = new Date(record.date).toISOString().split('T')[0]; // 转换为 YYYY-MM-DD 格式
...
// 心理测试记录的排序功能
const sortByScore = () => {
  filteredTestRecords.value.sort((a, b) => {
