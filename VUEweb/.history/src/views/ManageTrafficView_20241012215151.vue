    ...
    <div class="chart-container">
      <div class="chart">
        ...
        <div class="chart-options">
          <button @click="toggleFeedbackMetric('count')">Feedback Count</button>
          <button @click="toggleFeedbackMetric('average')">Average Ratings</button>
        </div>
      ...
      <div class="chart">
        ...
        <div class="chart-options">
          <button @click="toggleHealthTestMetric('count')">Health Test Count</button>
          <button @click="toggleHealthTestMetric('average')">Average Score</button>
        </div>
...
export default {
  ...
  setup() {
    ...
    const currentFeedbackMetric = ref('count');
    const currentHealthTestMetric = ref('count');
      ...
      try {
        ...
        querySnapshot.docs.forEach(doc => {
          ...
          feedback.ratings.forEach((ratingObj, index) => {
            if (!ratingsSum[formattedDate]) {
              ratingsSum[formattedDate] = 0;
              ratingsCount[formattedDate] = 0;
            }
            ratingsSum[formattedDate] += ratingObj.rating;
            ratingsCount[formattedDate]++;
          });
        });

        feedbackLabels.value = Object.keys(counts);
        feedbackPoints.value = Object.values(counts);
        feedbackAverageRatings.value = Object.keys(ratingsSum).map(date => (ratingsSum[date] / ratingsCount[date]).toFixed(2));
      } catch (error) {
    ...
    const toggleFeedbackMetric = (metric) => {
      currentFeedbackMetric.value = metric;
      const ctx = document.getElementById('feedbackChart').getContext('2d');
      const labels = feedbackLabels.value;
      const data = metric === 'count' ? feedbackPoints.value : feedbackAverageRatings.value;
      const label = metric === 'count' ? 'Daily Feedback Count' : 'Daily Average Feedback Rating';
      const type = metric === 'count' ? 'bar' : 'line';
      const borderColor = metric === 'count' ? 'rgba(75, 192, 192, 1)' : 'rgba(255, 206, 86, 1)';
      const backgroundColor = metric === 'count' ? 'rgba(75, 192, 192, 0.2)' : 'rgba(255, 206, 86, 0.2)';
      createChart(ctx, labels, data, label, type, borderColor, backgroundColor);
    };

    const toggleHealthTestMetric = (metric) => {
      currentHealthTestMetric.value = metric;
      const ctx = document.getElementById('healthTestsChart').getContext('2d');
      const labels = healthTestsLabels.value;
      const data = metric === 'count' ? healthTestsPoints.value : healthTestsAverageScores.value;
      const label = metric === 'count' ? 'Daily Health Test Count' : 'Daily Average Health Test Score';
      const type = metric === 'count' ? 'bar' : 'line';
      const borderColor = metric === 'count' ? 'rgba(153, 102, 255, 1)' : 'rgba(54, 162, 235, 1)';
      const backgroundColor = metric === 'count' ? 'rgba(153, 102, 255, 0.2)' : 'rgba(54, 162, 235, 0.2)';
      createChart(ctx, labels, data, label, type, borderColor, backgroundColor);
    };
    ...
    onMounted(async () => {
      ...
      toggleFeedbackMetric(currentFeedbackMetric.value);
      toggleHealthTestMetric(currentHealthTestMetric.value);
    ...
    return {
      toggleFeedbackMetric,
      toggleHealthTestMetric
    };
  }
};
</script>
