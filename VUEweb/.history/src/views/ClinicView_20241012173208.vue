...
export default {
  ...
  methods: {
    ...
    async fetchNearbyClinics(lat, lng) {
      try {
        const response = await axios.get(
          `https://api.mapbox.com/geocoding/v5/mapbox.places/clinic.json?proximity=${lng},${lat}&limit=10&access_token=${mapboxgl.accessToken}`
        );
    ...
    async selectClinic(clinic) {
      ...
      if (this.userCoordinates) {
        ...
        try {
          ...
          // Move the map to center on the selected clinic
          this.map.setCenter(clinicCoordinates);
          this.map.setZoom(14);
        } catch (error) {
