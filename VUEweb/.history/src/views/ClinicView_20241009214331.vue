...
const startCoords = ref([]) // 存储起点坐标
const endCoords = ref([])   // 存储终点坐标
...
const setupMap = () => {
  ...
  map.value.on('click', onMapClick); // 添加点击事件
}
...
const onMapClick = (e) => {
  ...
  if (!startCoords.value.length) {
    startCoords.value = coordinates; // 设置起点
    startLocation.value = 'Start location set';
  } else {
    endCoords.value = coordinates; // 设置终点
    endLocation.value = 'End location set';
  }

  new mapboxgl.Marker().setLngLat(coordinates).addTo(map.value); // 添加标记
}

// 获取导航路线
const getRoute = async () => {
  const apiUrl = `https://api.mapbox.com/directions/v5/mapbox/driving/${startCoords.value.join(',')};${endCoords.value.join(',')}?access_token=YOUR_MAPBOX_ACCESS_TOKEN`;
...
const getPlaceSuggestions = async () => {
  ...
  const apiUrl = `https://api.mapbox.com/geocoding/v5/mapbox.places/${encodeURIComponent(searchQuery.value)}.json?access_token=YOUR_MAPBOX_ACCESS_TOKEN&autocomplete=true`;
  try {
    const response = await axios.get(apiUrl)
...
const fetchNearbyClinics = async (coordinates) => {
  const apiUrl = `https://api.mapbox.com/geocoding/v5/mapbox.places/doctor.json?proximity=${coordinates[0]},${coordinates[1]}&access_token=YOUR_MAPBOX_ACCESS_TOKEN`;
  try {
    const response = await axios.get(apiUrl);
...
const searchGP = async () => {
  ...
  const apiUrl = `https://api.mapbox.com/geocoding/v5/mapbox.places/${encodeURIComponent(searchQuery.value)}.json?access_token=YOUR_MAPBOX_ACCESS_TOKEN`;
  try {
    const response = await axios.get(apiUrl)
