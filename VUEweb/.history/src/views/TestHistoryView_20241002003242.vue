...
const getTestRecords = async () => {
  try {
    ...
    if (startDate.value && endDate.value) {
      const startTimestamp = Timestamp.fromDate(new Date(startDate.value + ' 00:00:00'));
      const endTimestamp = Timestamp.fromDate(new Date(endDate.value + ' 23:59:59'));
    ...
    // 将获取到的数据映射到 testRecords 中
    testRecords.value = querySnapshot.docs.map(doc => doc.data());

    // 如果有记录，计算平均分
    if (testRecords.value.length > 0) {
      averageScore.value = testRecords.value.reduce((acc, record) => acc + record.score, 0) / testRecords.value.length;
    } else {
