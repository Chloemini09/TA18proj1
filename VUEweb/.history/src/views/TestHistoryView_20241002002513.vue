...
const getTestRecords = async () => {
  ...
  if (startDate.value && endDate.value) {
    ...
    // 注意这里的 `endTimestamp` 我们会稍微增加一天来确保查询包括结束日期
    const adjustedEndTimestamp = Timestamp.fromDate(new Date(new Date(endDate.value).setDate(new Date(endDate.value).getDate() + 1)))
