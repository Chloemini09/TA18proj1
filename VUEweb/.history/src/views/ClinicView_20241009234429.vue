...
const getRoutes = async () => {
  ...
  // 设置 selectedClinic 为用户自定义的地点，用于显示交通方式选择
  selectedClinic.value = {
    id: 'user-input', // 使用一个特殊的 ID 表示用户输入的地点
...
.header {
  display: flex;
  align-items: center;
  justify-content: space-between; /* 在logo和用户名之间保持左右分布 */
}

.logo {
  width: 80px; /* 减小 logo 尺寸，使其更协调 */
  height: auto;
  margin-right: 20px;
}
...
.user-info {
  display: flex;
  align-items: baseline; /* 使用户名和日期对齐基线 */
}

.username {
  font-size: 2rem;
  font-weight: bold;
  margin-right: 10px; /* 给用户名和日期之间增加间距 */
}

.current-date {
  font-size: 1rem;
  color: #666; /* 使用较淡的颜色，使得日期视觉上不抢眼 */
}
