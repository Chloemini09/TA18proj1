<template>
  <div class="user-courses">
    ...
    <div class="latest-videos">
      ...
      <div class="channel-section" v-for="channel in channels" :key="channel">
        <h3>{{ channel }}</h3>
        <div class="video-grid">
          <div class="video-card" v-for="course in getLatestVideos(channel)" :key="course.id">
            ...
            <div class="actions">
              <button @click="likeCourse(course)" class="btn-like">Like</button>
              <button @click="commentCourse(course)" class="btn-comment">Comment</button>
            </div>
          </div>
        </div>
      </div>
...
export default {
  setup() {
    ...
    const getLatestVideos = (channel) => {
      if (channel === 'All') {
        return courses.value.slice(-3).reverse();
      }
      return courses.value.filter(course => course.category === channel).slice(-3).reverse();
    };

    const likeCourse = (course) => {
      alert(`Liked course: ${course.title}`);
      // Logic to save like count to Firebase can be added here
    };

    const commentCourse = (course) => {
      const comment = prompt('Enter your comment');
      if (comment) {
        alert(`Commented on course: ${course.title}`);
        // Logic to save comment to Firebase can be added here
      }
    };
