...
const getTestRecords = async () => {
  ...
  if (startDate.value && endDate.value) {
    const startTimestamp = Timestamp.fromDate(new Date(new Date(startDate.value).setHours(0, 0, 0, 0))); // 开始日期第一秒
    const endTimestamp = Timestamp.fromDate(new Date(new Date(endDate.value).setHours(23, 59, 59, 999))); // 结束日期最后一秒
