  ...
  <div class="diary-list-container">
    ...
    <div class="diary-grid">
      <div class="diary-item" v-for="(diary, index) in paginatedDiaries" :key="diary.date">
        ...
        <!-- 根据 showMore[index] 控制内容显示 -->
        <p><strong>Content:</strong> 
          <span v-if="!showMore[index]">
            <span class="content-preview">{{ diary.content }}</span> 
            <button @click="toggleShowMore(index)" class="toggle-btn">Show More</button>
          </span>
          <span v-else>
            <span class="content-full">{{ diary.content }}</span>
            <button @click="toggleShowMore(index)" class="toggle-btn">Show Less</button>
          </span>
        </p>
      </div>
    </div>
...
const getDiaries = async () => {
  ...
  // 初始化 showMore 数组，所有项默认不展开
  showMore.value = Array(diaries.value.length).fill(false)
}
getDiaries()
