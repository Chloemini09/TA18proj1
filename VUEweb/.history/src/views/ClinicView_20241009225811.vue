      ...
      <div class="clinic-list">
        ...
        <ul>
          <li v-for="clinic in nearbyClinics" :key="clinic.id">
            ...
            <button @click="showRecommendedRoute(clinic)" class="recommend-btn">推荐最佳路线</button>
...
const showRecommendedRoute = async (clinic) => {
  ...
  if (bestMode) {
    // 显示推荐路线
    await fetchRoutesFromAPI(startCoords, endCoords, bestMode);
    alert(`推荐的交通方式是: ${bestMode === 'driving' ? '驾车' : bestMode === 'walking' ? '步行' : '骑行'}`);
  } else {
    alert('无法获取推荐的交通方式');
  }
};
