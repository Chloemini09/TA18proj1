  ...
  methods: {
    ...
    async searchAddress() {
      if (this.searchQuery.length > 2) {
        const response = await fetch(`https://api.mapbox.com/geocoding/v5/mapbox.places/${encodeURIComponent(this.searchQuery)}.json?access_token=${mapboxgl.accessToken}&country=AU&proximity=144.9631,-37.8136`);
    ...
    async findNearbyClinics(center) {
      const response = await fetch(`https://api.mapbox.com/geocoding/v5/mapbox.places/clinic.json?proximity=${center[0]},${center[1]}&radius=5000&access_token=${mapboxgl.accessToken}&country=AU`);
      const data = await response.json();
      this.nearbyClinics = data.features;
    ...
    async getRoute(mode) {
      ...
      const response = await fetch(`https://api.mapbox.com/directions/v5/mapbox/${mode}/${start};${end}?steps=true&geometries=geojson&access_token=${mapboxgl.accessToken}`);
      const data = await response.json();
