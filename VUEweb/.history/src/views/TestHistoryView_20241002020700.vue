...
const emailReport = async () => {
  const csvContent = generateCSV();
  console.log("Generated CSV Content:", csvContent); // 打印 CSV 内容
  const sendCSVReport = httpsCallable(functions, 'sendCSVReport');

  try {
    console.log("Calling sendCSVReport function with:", { email: user.email, csvContent });
    const result = await sendCSVReport({ email: user.email, csvContent });
