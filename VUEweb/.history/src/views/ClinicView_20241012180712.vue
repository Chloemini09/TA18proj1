<template>
  <div class="clinic-view">
    <div class="sidebar">
      ...
      <div v-if="nearbyFacilities.length" class="facility-list">
        <h3>Nearby Medical Facilities</h3>
        <ul>
          <li
            v-for="facility in paginatedFacilities"
            :key="facility.id"
            @click="showRoute(facility)"
            class="facility-item"
          >
            {{ facility.text }}
          </li>
      ...
      <div v-if="selectedFacility" class="route-info">
        <h3>Route to {{ selectedFacility.text }}</h3>
...
export default {
  ...
  data() {
    return {
      ...
      currentPage: 1,
      itemsPerPage: 5,
      mapboxToken: 'pk.eyJ1IjoiY2hsb2V5dWUiLCJhIjoiY20xejJpb3BhMDJndTJrcHpwaG9ocHE0NyJ9.jHaWplmk0Y_XWTLCtuAm4A'
    };
  },
  computed: {
    totalPages() {
  ...
  methods: {
    ...
    async findNearbyFacilities(center) {
      try {
        ...
        this.nearbyFacilities.forEach(facility => {
          this.addMarker(facility.center, 'facility');
        });
        this.currentPage = 1;
      } catch (error) {
    ...
    async getRoute(mode) {
      ...
      const end = this.selectedFacility.center.join(',');
      try {
