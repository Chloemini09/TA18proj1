    ...
    <div class="consultation-management">
      ...
      <section class="doctor-form">
        ...
        <form @submit.prevent="isEditing ? updateDoctor() : addDoctor()">
          ...
          <select v-model="doctor.workingDays" multiple required>
      ...
      <section class="doctor-list">
        ...
        <div class="search-section">
          ...
          <input type="text" v-model="searchTerm" placeholder="Search..." @input="filterDoctors" />
        </div>
        <div v-if="paginatedDoctors.length === 0">No doctors found.</div>
        <table v-else>
          <thead>
            <tr>
              <th>Name</th>
  ...
  const filterDoctors = () => {
    filteredDoctors.value = doctors.value.filter(doc => {
      ...
      if (searchCategory.value === 'name') {
        ...
        return doc.workingDays.some(day => day.toLowerCase().includes(term));
