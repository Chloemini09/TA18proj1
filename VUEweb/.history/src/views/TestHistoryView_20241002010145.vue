...
const filteredTestRecords = computed(() => {
  return testRecords.value.filter(record => {
    ...
    const startDateTime = startDate.value ? new Date(startDate.value) : null
    const endDateTime = endDate.value ? new Date(new Date(endDate.value).setDate(new Date(endDate.value).getDate() + 1)) : null
