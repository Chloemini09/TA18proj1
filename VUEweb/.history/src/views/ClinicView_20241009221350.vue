      ...
      <div class="clinic-list">
        ...
        <ul>
          <li v-for="clinic in nearbyClinics" :key="clinic.id">
            ...
            {{ clinic.address }}<br>
            <small>电话信息不可用</small>
            <button @click="navigateToMap(clinic.coordinates)">GO</button> <!-- 新增GO按钮 -->
          </li>
        </ul>
      </div>
    </div>
...
const getCoordinatesFromAddress = async (address) => {
  const apiUrl = `https://api.mapbox.com/geocoding/v5/mapbox.places/${encodeURIComponent(address)}.json?access_token=YOUR_MAPBOX_ACCESS_TOKEN`;
  try {
    const response = await axios.get(apiUrl);
...
// 跳转到新界面
const navigateToMap = async (clinicCoordinates) => {
  const startCoords = await getCoordinatesFromAddress(startLocation.value); // 获取起点坐标
  router.push({ name: 'MapView', params: { startCoords, endCoords: clinicCoordinates } });
}
