...
const analyzeTestResults = httpsCallable(functions, 'analyzeTestResults');
...
const getAIAnalysis = async () => {
  try {
    // 生成 CSV 数据
    const testResults = generateCSV(); // 生成 CSV 数据

    // 将 CSV 数据转为字符串并传递给云函数
    const response = await analyzeTestResults({ testResults });
    aiRecommendation.value = response.data.suggestion;
  } catch (error) {
    console.error("Error getting AI analysis:", error);
    aiRecommendation.value = "Unable to provide AI analysis at the moment. Please try again later.";
  }
};
